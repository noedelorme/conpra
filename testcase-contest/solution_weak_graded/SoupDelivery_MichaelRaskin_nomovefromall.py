...
def cheapest_single(fc, dc):
    opt = -1
    optcost = math.inf

    for f, cost in enumerate(fc):
        cost += sum(dc[f])
        if cost < optcost:
            opt = f
            optcost = cost
...
def best_chosen(used, dc):
    chosen = {}
    for c in range(len(dc[0])):
        opt = -1
        optcost = math.inf
        for f in used:
            if dc[f][c] < optcost:
...
def handle_case():
    ...
    cs = cheapest_single(fc, dc)

    used = {f: True for f in range(n)}
    chosen = best_chosen(used, dc)
    improved = True
    epsilon = 1/(2*(m+n)**2)
    while improved:
